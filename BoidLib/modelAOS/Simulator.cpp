...
namespace modelAOS
{
    ...
    void Simulator::NextStateSequentialKD()
    {
    ...
    void Simulator::NextStateParallelBarrier()
    {
#pragma omp parallel default(none) shared(boids, options, velocitiesX, velocitiesY)
        {
#pragma omp for schedule(dynamic)
            for (int i = 0; i < options.BoidNum; ++i)
            {
                const auto current = &boids[i];
                float xpos_avg = 0, ypos_avg = 0, xvel_avg = 0, yvel_avg = 0, close_dx = 0, close_dy = 0;
                int neighboring_boids = 0;
...
#pragma omp for
    ...
    void Simulator::NextStateParallelNoBarrier()
    {
        // Copy the boids to a new array
#pragma omp parallel default(none) shared(boids, options, velocitiesX, velocitiesY, positionX, positionY)
        {
#pragma omp for
            for (int i = 0; i < options.BoidNum; ++i)
            {
...
#pragma omp for schedule(dynamic)
    ...
    void Simulator::NextStateParallelKDNoBarrier()
    {
        const auto kdTree = std::make_unique<ModelAOS::KDTree>(boids, options.BoidNum, options.VisualRange);
...
#pragma omp for schedule(dynamic)
            for (int i = 0; i < options.BoidNum; ++i)
            {
                ...
                {
                    // access shared arrays so we don't have to take care of race conditions
    ...
    void Simulator::NextStateParallelKDBarrier()
    {
        // Copy the boids to a new array
        const auto kdTree = std::make_unique<ModelAOS::KDTree>(boids, options.BoidNum, options.VisualRange);
#pragma omp parallel default(none) shared(boids, options, kdTree, velocitiesX, velocitiesY)
        {
#pragma omp for schedule(dynamic)
            for (int i = 0; i < options.BoidNum; ++i)
            {
                ...
                const auto neighbors = kdTree->query(current);
